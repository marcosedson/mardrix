...
## Mardrix ERP (BFF + Guards + Tests)

This project now runs ERP flows through a BFF contract on `/api`, with optional proxying to a real backend.
...
2. Keep frontend calling only `/api/*` endpoints; set `BACKEND_API_URL` to enable proxy mode.
...
* Login (`tests/critical-flows/login-flow.test.ts`)
* Product CRUD (`tests/critical-flows/products-crud-flow.test.ts`)
* Accounts payable/receivable (`tests/critical-flows/finance-payable-receivable-flow.test.ts`)
